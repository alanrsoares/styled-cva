...
| Path                       | Package                       | Notes                                   |
| -------------------------- | ----------------------------- | --------------------------------------- |
| `packages/core`            | `@styled-cva/core`            | Shared `cva`, `cn`, helpers; no UI deps |
...
| `bun run test`  | All packages' vitest suites                           |
...
- **Commits**: Conventional Commits with optional scope. Common scopes:
  `(react)`, `(solid)`, `(vue)`, `(core)`, `(eslint-plugin)`, `(types)`,
...
- **Lint configs**: each package has its own `.eslintrc.json` (legacy
  ESLint 8). The repo doesn't use a flat config.
